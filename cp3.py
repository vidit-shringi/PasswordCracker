...
def display_students():
    ...
    for i in range(len(students)):
        for j in range(len(students[i])):
            tk.Label(window, text=students[i][j]).grid(row=i+1, column=j+4)
...
# Add display screen
tk.Label(window, text="Student Information", font=("Arial", 16)).grid(row=0, column=4, columnspan=4)
tk.Label(window, text="ID").grid(row=1, column=4)
tk.Label(window, text="Name").grid(row=1, column=5)
tk.Label(window, text="Age").grid(row=1, column=6)
tk.Label(window, text="Grade").grid(row=1, column=7)
